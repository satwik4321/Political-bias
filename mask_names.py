...
def process_data_concurrently(df, max_workers=1):
    with ThreadPoolExecutor(max_workers=1) as executor:
        future_to_index = {executor.submit(mask_text, row['content']): idx for idx, row in df.iterrows()}
        for future in as_completed(future_to_index):
            idx = future_to_index[future]
            try:
                df.at[idx, 'ner masked data'] = future.result()
...
output_columns = ['content', 'bias','ner masked data']
df = df[output_columns]

# Save to CSV
df.to_csv(r"C:\Users\sathw\Documents\Masking tasks\Masked output2.csv", index=False)
